...
| `server.py` 执行 `close()` 之后 |TIME_WAIT / CLOSE_WAIT |连接关闭阶段 |
...
| 项目 | 你的填写 |
| :--- | :------- |
| 连接的目标地址和端口 |180.163.210.217:80 |
| 响应的第一行（状态行） |HTTP/1.1 200 OK |
| 程序是否自动退出（不需要手动 Ctrl+C）|是 |
| 是你调用 `close()` 触发的断开，还是服务器主动断开的 |是我调用 close() 触发的断开 |
...
| :--- | :------- | :------- |
| 程序是否自动退出 |是 |否 |
| `ss` 中连接的 State |TIME_WAIT |ESTABLISHED (或 CLOSE_WAIT) |
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：
   客户端会抛出 ConnectionRefusedError（连接被拒绝） 错误。客户端运行时会成功创建套接字（socket() 调用正常执行），因此套接字是存在的；但由于服务器未启动监听，无法完成 TCP 三次握手建立连接，因此通信管道（虚拟连接）不存在，客户端的连接请求会被操作系统直接拒绝。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：
   server 是监听套接字，核心职责是持续监听指定端口，等待新的客户端连接请求，不负责具体数据传输；conn 是已连接套接字，专门用于与单个已连接的客户端进行双向数据收发。accept() 返回新套接字，是为了实现监听与通信分离：既保证监听套接字持续接收新连接，让服务器可同时处理多个客户端，又避免多个客户端的通信数据在同一个套接字上混淆，保证数据传输的独立性。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：
本质区别：描述符是操作系统为进程分配的本地文件句柄，仅在当前主机、当前进程内唯一；端口号是 TCP/IP 协议定义的网络层标识，是全网范围内的通信标识。
各自作用：描述符用于进程快速定位内核中的套接字对象，实现进程与内核的交互；端口号用于在网络中区分同一主机上的不同网络应用，让一台主机可同时运行多个网络服务，保证数据准确送达对应应用。


4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：
   client.py 中仅指定了服务器的 IP 和端口，未手动指定客户端自身的端口号。客户端端口由操作系统内核自动分配，属于临时 / 动态端口，范围通常为 1024~65535。在 ss（Windows 为netstat）的输出中，ESTABLISHED 状态的连接行，本地地址列冒号后的数字，就是分配给客户端的临时端口，因此可以看到。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：
   HTTP/1.0 默认采用短连接模型：服务器处理完客户端请求、发送完完整响应后，会主动发送FIN包关闭 TCP 连接。当客户端recv()读取完服务器发送的最后一个数据后，再次调用recv()时，会收到「连接已关闭」的信号（返回空字节流），因此recv()循环会自动退出，无需手动调用close()。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：
   二者的根本区别在于默认连接模型：
HTTP/1.0：默认使用短连接（Connection: close），服务器发送完响应后主动断开 TCP 连接，客户端recv()会收到连接结束信号，循环自动退出。
HTTP/1.1：默认使用长连接（Connection: keep-alive），服务器不会主动断开连接，而是保持ESTABLISHED状态，等待客户端发送新的请求。因此客户端recv()会一直阻塞等待新数据，程序卡住，必须手动Ctrl+C终止。
